...
(function( $ ) {
	...
	$.redux.ajax_save = function( button ) {
		...
		var overlay           = $( document.getElementById( 'Redux_Travelpayouts_ajax_overlay' ) );
		var $notification_bar = $( document.getElementById( 'Redux_Travelpayouts_notification_bar' ) );
		...
		$( '.Redux_Travelpayouts_ajax_save_error' ).slideUp(
			'medium',
			function() {
				$( this ).remove();
			}
		...
		if ( 'Redux_Travelpayouts_save' !== button.attr( 'name' ) ) {
			...
			{ type: 'post',
				...
				success: function( response ) {
					...
					if ( response.action && 'reload' === response.action ) {
						...
						$save_notice = $( document.getElementById( 'Redux_Travelpayouts_notification_bar' ) ).find( '.saved_notice' );
						...
						$( '.redux-action_bar .spinner' ).removeClass( 'is-active' );
						overlay.fadeOut( 'fast' );
						$( '.wrap h2:first' ).parent().append( '<div class="error Redux_Travelpayouts_ajax_save_error" style="display:none;"><p>' + response.status + '</p></div>' );
						$( '.Redux_Travelpayouts_ajax_save_error' ).slideDown();
						$( 'html, body' ).animate(
							{ scrollTop: 0 },
							'slow'
						);
