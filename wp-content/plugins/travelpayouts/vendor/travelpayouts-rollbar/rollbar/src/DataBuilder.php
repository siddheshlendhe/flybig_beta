...
use Travelpayouts\Vendor\Rollbar\Payload\Context;
use Travelpayouts\Vendor\Rollbar\Payload\Message;
use Travelpayouts\Vendor\Rollbar\Payload\Body;
use Travelpayouts\Vendor\Rollbar\Payload\Level;
use Travelpayouts\Vendor\Rollbar\Payload\Person;
use Travelpayouts\Vendor\Rollbar\Payload\Server;
use Travelpayouts\Vendor\Rollbar\Payload\Request;
use Travelpayouts\Vendor\Rollbar\Payload\Data;
use Travelpayouts\Vendor\Rollbar\Payload\Trace;
use Travelpayouts\Vendor\Rollbar\Payload\Frame;
use Travelpayouts\Vendor\Rollbar\Payload\TraceChain;
use Travelpayouts\Vendor\Rollbar\Payload\ExceptionInfo;
use Travelpayouts\Vendor\Rollbar\Rollbar;
use Travelpayouts\Vendor\Rollbar\Exceptions\PersonFuncException;
...
class DataBuilder implements DataBuilderInterface
{
    ...
    {
        $this->custom = isset($config['custom']) ? $config['custom'] : \Travelpayouts\Vendor\Rollbar\Defaults::get()->custom();
    }
    
    public function setCustomDataMethod($config)
    {
        $this->customDataMethod = isset($config['custom_data_method']) ?
            $config['custom_data_method'] : \Travelpayouts\Vendor\Rollbar\Defaults::get()->customDataMethod();
    }
     ...
     * @var string $errline
     *
     * @return Travelpayouts\Vendor\Rollbar\ErrorWrapper
     */
    ...
    private function stripShutdownFrames($backTrace)
    {
        foreach ($backTrace as $index => $frame) {
            ...
            $fatalHandlerMethod = (isset($method)
                                    && $method === 'Travelpayouts\\Vendor\\Rollbar\Handlers\\FatalHandler::handle');
                                    
            $fatalHandlerClassAndFunction = (isset($class)
                                                && $class === 'Travelpayouts\\Vendor\\Rollbar\Handlers\\FatalHandler'
                                                && isset($function)
                                                && $function === 'handle');
            
            $errorHandlerMethod = (isset($method)
                                    && $method === 'Travelpayouts\\Vendor\\Rollbar\Handlers\\ErrorHandler::handle');
                                    
            $errorHandlerClassAndFunction = (isset($class)
                                                && $class === 'Travelpayouts\\Vendor\\Rollbar\Handlers\\ErrorHandler'
                                                && isset($function)
                                                && $function === 'handle');
