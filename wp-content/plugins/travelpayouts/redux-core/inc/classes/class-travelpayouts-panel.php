...
if ( ! class_exists( 'Redux_Travelpayouts_Panel', false ) ) {
	...
	class Redux_Travelpayouts_Panel {
		...
		private function panel_template() {
			...
			// Security is vital!
			echo '<input type="hidden" class="redux-ajax-security" data-opt-name="' . esc_attr( $this->parent->args['opt_name'] ) . '" id="ajaxsecurity" name="security" value="' . esc_attr( wp_create_nonce( 'Redux_Travelpayouts_ajax_nonce' . $this->parent->args['opt_name'] ) ) . '" />';
			/**
