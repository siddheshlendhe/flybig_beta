...
use Travelpayouts\Vendor\Rollbar\Truncation\Truncation;
use Travelpayouts\Vendor\Monolog\Logger as MonologLogger;
...
class RollbarLogger extends AbstractLogger
{
    ...
    public function log($level, $toLog, array $context = array(), $isUncaught = false)
    {
        ...
        if (!$this->levelFactory->isValidLevel($level)) {
            $exception = new \Travelpayouts\Vendor\Psr\Log\InvalidArgumentException("Invalid log level '$level'.");
    ...
    protected function send(\Travelpayouts\Vendor\Rollbar\Payload\EncodedPayload $payload, $accessToken)
    {
     ...
     */
    protected function truncate(\Travelpayouts\Vendor\Rollbar\Payload\EncodedPayload &$payload)
    {
